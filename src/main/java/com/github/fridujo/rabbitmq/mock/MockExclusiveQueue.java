...
public class MockExclusiveQueue extends MockQueue {
    ...
    void close(MockConnection mockConnection) {
        ...
        } else if (pointer().name().startsWith("amq.gen-")) {
            LOGGER.debug("not closing exclusive queue with autogenerated name");
        } else {
            LOGGER.warn("RESOURCE_LOCKED - unable to close exclusive queue from another connection");
        }
    }
